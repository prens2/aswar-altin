        ...
        const arabicNews = [
            {
                ...
                urlToImage: "https://images.unsplash.com/photo-1580330069902-0c8c3e19a8a1?w=300",
            ...
            },
            {
                ...
                urlToImage: "https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?w=300",
...
// 🔥 دالة تحديث عرض الأخبار
function updateNewsDisplay() {
    const newsContainer = $("#newsContainer");
    if (!newsContainer) return;
    
    if (goldNews.length === 0) {
        newsContainer.innerHTML = `<div class="news-item"><p>${currentLanguage === 'ar' ? 'جاري تحميل الأخبار...' : currentLanguage === 'en' ? 'Loading news...' : 'Haberler yükleniyor...'}</p></div>`;
        return;
    }
    
  newsContainer.innerHTML = goldNews.map(news => `
        <div class="news-item">
            <img src="${news.urlToImage}" alt="" class="news-image">
            <div class="news-content">
                <h4>${news.title[currentLanguage] || news.title.ar}</h4>
                <p>${news.description[currentLanguage] || news.description.ar}</p>
                <div class="news-source">${news.source[currentLanguage] || news.source.ar}</div>
                <small>${new Date(news.publishedAt).toLocaleDateString(currentLanguage === 'ar' ? 'ar-EG' : currentLanguage === 'en' ? 'en-US' : 'tr-TR')}</small>
            </div>
        </div>
    `).join('');
}
